...
contract('HumaniqICO', function(accounts) {
    ...
    it("Should finish crowdsale and allocate founder bonus", function() {
        // ICO Contract
        var icoContract = HumaniqICO.deployed();
        // Token contract
        var tokenContract = HumaniqToken.deployed();
        ...
        return icoContract.finishCrowdsale({
            ...
            assert.equal(5, icoTotalBalance);
            return icoContract.coinsIssued.call();
        }).then(function(coinsIssued) {

            var totalCoinsIssued = coinsIssued.toNumber();
            var founderBonus = (coinsIssued / 86) * 14;
            return tokenContract.balanceOf.call(multisig).then(function(balance) {
                assert.equal(founderBonus, founderTokens,
                    "Founders were not allocated with proper number of coins");
            });
        });
    });
});
